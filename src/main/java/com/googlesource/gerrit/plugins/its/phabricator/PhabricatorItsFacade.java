...
public class PhabricatorItsFacade implements ItsFacade {
    ...
    if (chopped.length >= 1) {
      ...
      switch (action) {
        case "add-project":
          ...
          assertManiphest(Object chopped, taskInfo, Conduit.ACTION_PROJECT_ADD);
          break;
        case "remove-project":
          assertParameters(action, chopped, 1);
          assertManiphest(chopped, taskInfo, Conduit.ACTION_PROJECT_REMOVE);
          break;
        default:
  ...
  private void assertManiphest(Object chopped, Object taskInfo, Object actions) {
    try {
      ...
      conduit.maniphestUpdate(taskId, projectPhids, actions);
    } catch (ConduitException e) {
