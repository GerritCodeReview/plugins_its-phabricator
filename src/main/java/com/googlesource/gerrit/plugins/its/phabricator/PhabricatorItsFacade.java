...
public class PhabricatorItsFacade implements ItsFacade {
    ...
    if (chopped.length >= 1) {
      ...
      switch (action) {
        case "add-project":
          ...
          assertManiphest(chopped, taskId, Conduit.ACTION_PROJECT_ADD);
          break;
        case "remove-project":
          assertParameters(action, chopped, 1);

          assertManiphest(chopped, taskId, Conduit.ACTION_PROJECT_REMOVE);
          break;
        default:
   ...
   * String actions
   */
  private void assertManiphest(String[] chopped, int taskId, String actions) throws IOException {
    try {
